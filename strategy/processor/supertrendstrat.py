...
def start_supertrend():
    ...
    histo=historical_price.objects.filter(yahoo_id=str('BTC'), spot_date__gte=start_date, spot_date__lte=end_date).values()
    dfHisto=pd.DataFrame(histo)
    print(dfHisto)
    resultat= get_supertrend(dfHisto['high_price'], dfHisto['low_price'], dfHisto['close_price'], 10, 3)
    return resultat
...
def get_supertrend(high, low, close, lookback, multiplier):
    ...
    supertrend = supertrend.dropna()[0:]
